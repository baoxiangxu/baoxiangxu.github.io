...
const DisplacementSphere = (props) => {
    ...
    useEffect(() => {
        ...
        // 128x128 segments is ~33k vertices pushed through a custom vertex
        // shader every frame — far more detail than this soft, blurred blob
        // needs, and it pegs the CPU on machines without GPU acceleration.
        // Scale the mesh density to the device instead.
        const segments =
            window.innerWidth <= media.mobile
                ? 48
                : window.innerWidth <= media.tablet
                ? 64
                : 80;
        geometry.current = new SphereBufferGeometry(32, segments, segments);
    ...
    useEffect(() => {
        ...
        // With motion reduced there is no animation loop, so paint one frame
        // here — otherwise a theme switch would leave the previous colours on
        // screen.
        if (prefersReducedMotion && renderer.current) {
    ...
    }, [rgbBackground, theme, prefersReducedMotion]);
